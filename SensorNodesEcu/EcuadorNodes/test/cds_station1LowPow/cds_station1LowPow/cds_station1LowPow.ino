...
const char *appKey = "DF93ECBF5F55BD9431C065A4E3A912FA";
...
#define TX_INTERVAL_SLEEP 410 //450*8 = 1200 sec = 60 min
//#define TX_INTERVAL_SLEEP 1 //1*8 = 8sec 
//#define TX_INTERVAL_SLEEP 15 //2*8 =16 sec
...
void setup()
{
  ...
  pinMode(POWER_PIN12,OUTPUT); //wind sensor is connected here
  digitalWrite(POWER_PIN12,HIGH);
  delay(40000); //for sleepingmode
...
void loop()
{
 ...
 uint32_t windSpeed=getWindSpeed()*100;
  ...
  //**** SLEEP THE NODE
  //Sleepin LORA module
    ttn.sleep(TX_INTERVAL_SLEEP*8*1000);
  ...
  //Sleeping sensors connected to POWER_PIN
   ...
   //********** Going to sleep power down ********/
    enterSleep(TX_INTERVAL_SLEEP); //sleep TX_INTERVAL_SLEEP*8 seconds
...
float getVis(){
  //There is not Vis sensor in this node
  return 0.0;
